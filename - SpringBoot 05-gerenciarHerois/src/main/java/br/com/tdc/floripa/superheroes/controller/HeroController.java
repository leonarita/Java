...
public class HeroController {
	...
	public Mono<ResponseEntity<Hero>> getHeroById(@PathVariable(value = "id") String heroId) {
	    return heroRepository.findById(heroId) .map(savedHero -> ResponseEntity.ok(savedHero)).defaultIfEmpty(ResponseEntity.notFound().build());
	}
	
	@PutMapping("/heroes/{id}")
	public Mono<ResponseEntity<Hero>> updateHero(@PathVariable(value = "id") String heroId, @Valid @RequestBody Hero hero) {
	    
		return heroRepository.findById(heroId).flatMap(existingHero -> { 
			existingHero.setName(hero.getName());
	        return heroRepository.save(existingHero);
	    })
		.map(updateHero -> new ResponseEntity<>(updateHero, HttpStatus.OK)).defaultIfEmpty(new ResponseEntity<>(HttpStatus.NOT_FOUND));
	  }
	  ...
	  public Mono<ResponseEntity<Void>> deleteHero(@PathVariable(value = "id") String heroId) {
	    ...
	    return heroRepository.findById(heroId).flatMap(existingHero ->
	    	heroRepository.delete(existingHero).then(Mono.just(new ResponseEntity<Void>(HttpStatus.OK)))
